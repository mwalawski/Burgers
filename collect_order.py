...
def create_table():
	c.execute('CREATE TABLE IF NOT EXISTS orders(ID INT, product REAL, amount REAL, hour REAL, phone TEXT)')


def dynamic_data_entry(id, product, amount, hour, phone):
	create_table()
	c.execute('''INSERT INTO orders(ID, product, amount, hour, phone)
				VALUES (?,?,?,?,?)''', (id, product, amount, hour, phone))
	conn.commit()


def check_availabilty(order_amount, burger_limit=8):
	current_time = (time.ctime(time.time())[11:16])
	c.execute('''SELECT hour
				FROM orders
				WHERE hour<?
				GROUP BY hour
				HAVING (SUM(amount)+?)<=?''', (current_time, order_amount, burger_limit))
	return [hour[0] for hour in c.fetchall()]


def checkout():
	order_id = input('Enter your order ID: ')
	c.execute('''SELECT SUM(p.price*o.amount)
				FROM products p, orders o
				WHERE o.ID=?
				AND o.product=p.ID''', (order_id))
